<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-bold">Edit Peminjaman Arsip</h2>
    </x-slot>

    <x-breadcrumb title="Edit Peminjaman Arsip" :items="[
    ...
    <div class="space-y-6">
        <div class="p-0 sm:p-8 sm:pb-12 bg-white shadow sm:rounded-lg dark:bg-gray-800">
            <form action="{{ route('archive-loan.update', $loan->id) }}" method="POST" class="px-2 py-2">
                @csrf
                @method('PUT')

                @php
                    $oldOrData = fn($field) => old($field, $loan->$field ?? '');
                @endphp
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <select name="user_id" id="user_id" class="form-select w-full">
                            ...
                            @foreach($users as $user)
                                <option value="{{ $user->id }}" 
                                        ...
                                        {{ $oldOrData('user_id') == $user->id ? 'selected' : '' }}>
                                    {{ $user->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="w-full">
                        ...
                        <x-form.input value="{{ $oldOrData('department') ?? $loan->user->departemen ?? '' }}" type="text" name="department" id="department" class="w-full bg-gray-100" readonly />
                    ...
                    <div class="w-full">
                        ...
                        <select name="archive_type" id="archive_type" class="form-select w-full">
                            ...
                            <option value="ga" {{ $oldOrData('archive_type') === 'ga' ? 'selected' : '' }}>GA Archive</option>
                            <option value="vendor" {{ $oldOrData('archive_type') === 'vendor' ? 'selected' : '' }}>Vendor Archive</option>
                        </select>
                    ...
                    <div class="w-full md:col-span-2" id="archive_ga" style="display:none">
                        ...
                        <select name="archive_id_ga" class="form-select w-full">
                            @foreach($gaArchives as $archive)
                                <option value="{{ $archive->id }}" {{ $oldOrData('archive_id') == $archive->id && $oldOrData('archive_type') === 'ga' ? 'selected' : '' }}>
                    ...
                    <div class="w-full md:col-span-2" id="archive_vendor" style="display:none">
                        ...
                        <select name="archive_id_vendor" class="form-select w-full">
                            @foreach($vendorArchives as $archive)
                                <option value="{{ $archive->id }}" {{ $oldOrData('archive_id') == $archive->id && $oldOrData('archive_type') === 'vendor' ? 'selected' : '' }}>
                    ...
                    <input type="hidden" name="archive_id" id="archive_id_real" value="{{ $oldOrData('archive_id') }}">

                    {{-- Tanggal Pinjam --}}
                    <div class="w-full">
                        <x-form.label for="loan_date" class="font-semibold py-1">Tanggal Pinjam</x-form.label>
                        <x-form.input value="{{ $oldOrData('loan_date') }}" type="date" name="loan_date" id="loan_date" class="w-full" />
                    ...
                    <div class="w-full">
                        ...
                        <x-form.input value="{{ $oldOrData('return_date') }}" type="date" name="return_date" id="return_date" class="w-full" />
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Set initial state (on page load) - important for edit form
            if (typeSelector.value === 'ga') {
